...
$('#new-user').addEventListener('click', () => $('#user-form').classList.toggle('hidden')); $('#save-user').addEventListener('click', async () => { const data = {full_name:$('#user-full-name').value, phone:$('#user-phone').value, role:$('#user-role').value, password:$('#user-password').value}; try { await api('/api/users', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(data)}); $('#user-form').classList.add('hidden'); toast('Пользователь добавлен'); loadUsers(); } catch(error) { toast(error.message); } });

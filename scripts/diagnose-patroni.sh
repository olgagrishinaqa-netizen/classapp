...
# 4. Check SCOPE/NAMESPACE consistency across pods (Spilo-generated Patroni config)
echo "4️⃣  Patroni scope/namespace consistency (etcd DCS identity):"
echo "Pod: scope / namespace / etcd hosts"
kubectl get pods -n "$NAMESPACE" -l app.kubernetes.io/name=patroni -o jsonpath='{range .items[*]}{.metadata.name}{": "}{.spec.containers[?(@.name=="patroni")].env[?(@.name=="SCOPE")].value}{" / "}{.spec.containers[?(@.name=="patroni")].env[?(@.name=="NAMESPACE")].value}{" / "}{.spec.containers[?(@.name=="patroni")].env[?(@.name=="ETCD_HOSTS")].value}{"\n"}{end}' 2>/dev/null || echo "⚠️  Could not read env vars (pods may not exist yet)"
echo ""

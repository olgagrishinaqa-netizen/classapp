...
def _connectable_has_table(connectable: Connection, tablename: str, schemaname: str | None) -> bool:
...
def _fk_is_self_referential(constraint: ForeignKeyConstraint) -> bool:
    ...
    return _fk_target_table_key(constraint.elements[0]) == constraint.parent.key
...
def _constraint_is_named(constraint: Constraint | Index, dialect: Dialect | None) -> bool:
    if constraint.name is None:
        return False
    assert dialect is not None
...
def all_table_check_constraints(table: Table) -> set[CheckConstraint]:
    ...
    those on columns."""
    candidates = chain(table.constraints, *(c.constraints for c in table.columns))

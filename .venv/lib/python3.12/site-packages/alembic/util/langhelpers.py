...
@overload
def to_tuple(x: Any, default: tuple[Any, ...] | None = None) -> tuple[Any, ...]: ...


def to_tuple(x: Any, default: tuple[Any, ...] | None = None) -> tuple[Any, ...] | None:
    if x is None:
        return default
...
class PriorityDispatcher:
    ...
    def __init__(self) -> None:
        self._registry: dict[tuple[Any, ...], Any] = collections.defaultdict(list)
    ...
    def dispatch_for(
        ...
        *,
        ...
        def decorate(fn: _C) -> _C:
            self._registry[(target, qualifier, priority)].append((fn, subgroup))
            return fn

        return decorate

    def dispatch(self, target: str, *, qualifier: str = "default") -> Callable[..., None]:
        ...
        def go(*arg: Any, **kw: Any) -> Any:
            ...
            for priority in DispatchPriority:
                for qualifier in qualifiers:
                    for fn, subgroup in self._registry.get((target, qualifier, priority), ()):
                        if (
                            results_by_subgroup.get(subgroup, PriorityDispatchResult.CONTINUE)

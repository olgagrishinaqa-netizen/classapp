...
def _compare_check_constraints(
    ...
    metadata_ck_constraints = sqla_compat.all_table_check_constraints(metadata_table)

    try:
        conn_ck_list = _InspectorConv(inspector).get_check_constraints(tname, schema=schema)
    ...
    conn_ck_objs = {_make_check_constraint(impl, ck_def, conn_table) for ck_def in conn_ck_list}
    ...
    conn_ck_sig = {impl._create_reflected_constraint_sig(ck) for ck in conn_ck_objs}

    metadata_ck_by_name = {
        c.name: c for c in metadata_ck_sig if sqla_compat.constraint_name_string(c.name)
    }
    conn_ck_by_name = {c.name: c for c in conn_ck_sig if sqla_compat.constraint_name_string(c.name)}

    for removed_name in sorted(set(conn_ck_by_name).difference(metadata_ck_by_name)):
        ...
        if autogen_context.run_object_filters(
            ...
            modify_table_ops.ops.append(ops.DropConstraintOp.from_constraint(conn_obj.const))
            log.info(
    ...
    for existing_name in sorted(set(metadata_ck_by_name).intersection(conn_ck_by_name)):
        ...
        if comparison.is_different:
            if autogen_context.run_object_filters(
                ...
                modify_table_ops.ops.append(ops.DropConstraintOp.from_constraint(conn_obj.const))
                modify_table_ops.ops.append(ops.AddConstraintOp.from_constraint(metadata_obj.const))
        elif comparison.is_skip:
            log.info(
                "Cannot compare check constraint %r, " "assuming equal and skipping. %s",
                existing_name,
                comparison.message,
            )

    for added_name in sorted(set(metadata_ck_by_name).difference(conn_ck_by_name)):

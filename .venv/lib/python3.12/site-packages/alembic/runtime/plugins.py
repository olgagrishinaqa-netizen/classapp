...
class Plugin:
    ...
    def populate_autogenerate_priority_dispatch(
        ...
        for plugin in _all_plugins.values():
            ...
            include_matches = [incl for incl in include if include[incl].match(plugin.name)]
        ...
        if never_matched:
            raise util.CommandError(f"Did not locate plugins: {', '.join(never_matched)}")
...
_legacy_plugin_names = {
    ...
    "alembic.autogenerate.checkconstraint_byname": ("alembic.ext.checkconstraint_byname"),
}

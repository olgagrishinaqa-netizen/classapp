...
def _compare_identity_default(
    ...
    if (
        ...
        or not sqla_compat._server_default_is_identity(metadata_default, conn_col_default)
    ):
        return PriorityDispatchResult.CONTINUE
...
def _user_compare_server_default(
    ...
    if is_diff:
        alter_column_op.modify_server_default = metadata_default
        log.info(
            "User defined function %s detected " "server default on column '%s.%s'",
...
def _dialect_impl_compare_server_default(
    ...
    if not isinstance(metadata_default, (DefaultClause, NoneType)) or not isinstance(
        conn_col_default, (DefaultClause, NoneType)
    ):
        return PriorityDispatchResult.CONTINUE

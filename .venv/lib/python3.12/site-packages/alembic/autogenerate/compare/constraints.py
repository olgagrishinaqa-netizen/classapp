...
def _compare_indexes_and_uniques(
    ...
    if conn_table is not None:
        conn_uniques_reflected: Collection[ReflectedUniqueConstraint] = frozenset()
        conn_indexes_reflected: Collection[ReflectedIndex] = frozenset()

        # 1b. ... and from connection, if the table exists
        try:
            conn_uniques_reflected = _InspectorConv(inspector).get_unique_constraints(
                tname, schema=schema
            )
        ...
        try:
            conn_indexes_reflected = _InspectorConv(inspector).get_indexes(tname, schema=schema)
        except NotImplementedError:
            pass
        else:
        ...
        conn_indexes = {
            index
            for index in (_make_index(impl, ix, conn_table) for ix in conn_indexes_reflected)
    ...
    conn_indexes_by_name: dict[sqla_compat._ConstraintName, _constraint_sig[sa_schema.Index]]
    ...
    def obj_added(
        obj: _constraint_sig[sa_schema.UniqueConstraint] | _constraint_sig[sa_schema.Index],
    ):
        if is_index_sig(obj):
    ...
    def obj_changed(
        old: _constraint_sig[sa_schema.UniqueConstraint] | _constraint_sig[sa_schema.Index],
        new: _constraint_sig[sa_schema.UniqueConstraint] | _constraint_sig[sa_schema.Index],
        msg: str,
    ):
        if is_index_sig(old):
...
def _make_index(impl: DefaultImpl, params: ReflectedIndex, conn_table: Table) -> Index | None:
...
def _make_foreign_key(
    ...
    remote_table_key = sqla_compat._get_table_key(referred_table, referred_schema)
    if remote_table_key not in conn_table.metadata:
        # create a placeholder table
        sa_schema.Table(
            referred_table,
            conn_table.metadata,
            schema=(referred_schema if referred_schema is not None else sa_schema.BLANK_SCHEMA),
            *[
                sa_schema.Column(remote, conn_table.c[local].type)
                for local, remote in zip(params["constrained_columns"], params["referred_columns"])
            ],
            info={"alembic_placeholder": True},
        )
    elif conn_table.metadata.tables[remote_table_key].info.get("alembic_placeholder"):
        ...
        for local, remote in zip(params["constrained_columns"], params["referred_columns"]):
            if remote not in placeholder_table.c:
                placeholder_table.append_column(sa_schema.Column(remote, conn_table.c[local].type))

...
class _InspectorConv:
    ...
    def get_indexes(self, tname: str, schema: str | None) -> list[ReflectedIndex]:
        raise NotImplementedError()
...
class _LegacyInspectorConv(_InspectorConv):
    ...
    def get_indexes(self, tname: str, schema: str | None) -> list[ReflectedIndex]:
        return self._apply_reflectinfo_conv(self.inspector.get_indexes(tname, schema=schema))
    ...
    def get_foreign_keys(
        ...
        return self._apply_reflectinfo_conv(self.inspector.get_foreign_keys(tname, schema=schema))
...
class _SQLA2InspectorConv(_InspectorConv):
    ...
    def _pre_cache(
        ...
        if (
            fraction is None
            or fraction <= 0.5
            or not self.inspector.dialect._overrides_default(inspector_method.__name__)
        ):
        ...
        try:
            elements = inspector_method(schema=schema, filter_names=optimized_filter_names)
    ...
    def _make_reflection_info(self, tname: str, schema: str | None) -> _ReflectionInfo:
        from sqlalchemy.engine.reflection import _ReflectionInfo

...
def _add_check_constraint(autogen_context: AutogenContext, op: ops.CreateCheckConstraintOp) -> str:
    constraint = op.to_constraint()
    args = [repr(_render_gen_name(autogen_context, op.constraint_name))]
    if not autogen_context._has_batch:
        args.append(repr(_ident(op.table_name)))
    args.append(_render_potential_expr(constraint.sqltext, autogen_context, wrap_in_element=False))
...
def _render_potential_expr(
    ...
    *,
    ...
    if isinstance(value, (list, tuple)):
        rendered = [_render_potential_expr(item, autogen_context) for item in value]

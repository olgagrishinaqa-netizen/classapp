...
class ApplyBatchImpl:
    ...
    def _setup_referent(self, metadata: MetaData, constraint: ForeignKeyConstraint) -> None:
        ...
        if tname != self.temp_table_name:
            ...
            colnames = [
                name
                for name in (_fk_target_tokens(elem).column_name for elem in constraint.elements)
                if name is not None
            ]
    ...
    def drop_column(
        ...
        **kw,
        ...
        if (
            ...
            self.named_constraints.pop(kw["existing_type"].name, None)  # type: ignore[attr-defined]

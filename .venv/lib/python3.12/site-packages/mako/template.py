...
class Template:
    ...
    def _compile_from_file(self, path, filename):
        if path is not None:
            ...
            with _translate_module_warnings(lambda: util.read_python_file(path), path, filename):
...
def _translate_module_warnings(get_source, module_id, filename):
    ...
    def _locate(message, category, warning_filename, lineno):
        ...
        if line_map is None:
            try:
                line_map = ModuleInfo.get_module_source_metadata(get_source(), full_line_map=True)[
                    "full_line_map"
                ]
            except Exception:
...
def _compile_text(template, text, filename):
    ...
    with _translate_module_warnings(lambda: source, cid, filename or template.uri):

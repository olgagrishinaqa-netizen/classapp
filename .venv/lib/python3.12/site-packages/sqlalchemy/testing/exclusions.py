...
class compound:
    ...
    def not_(self):
        ...
        copy.warns.update({NotPredicate(warn): element for warn, element in self.warns.items()})
        return copy
    ...
    def matching_warnings(self, config):
        return [
            message for predicate, (message, assert_) in self.warns.items() if predicate(config)
        ]
            ...
            def _expect_warnings():
                with contextlib.ExitStack() as stack:
                    for expression, assert_ in self.warns.values():
                        stack.enter_context(expect_warnings(expression, assert_=assert_))
                    yield

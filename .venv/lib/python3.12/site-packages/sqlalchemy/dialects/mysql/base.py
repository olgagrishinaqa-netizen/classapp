                    ...
                    if isinstance(element, elements.BindParameter) and element.type._isnull:
                        ...
                        if requires_mysql8_alias:
                            column_literal_clause = (
                                f"{_on_dup_alias_name}." f"{self.preparer.quote(element.name)}"
                            )
                        else:
...
class MySQLDDLCompiler(compiler.DDLCompiler):
    ...
    def visit_create_index(self, create: ddl.CreateIndex, **kw: Any) -> str:  # type: ignore[override]  # noqa: E501
        ...
        parser = index.get_dialect_option(self.dialect, "with_parser", deprecated_fallback="mysql")
        if parser is not None:
            text += " WITH PARSER %s" % (parser,)

        using = index.get_dialect_option(self.dialect, "using", deprecated_fallback="mysql")
    ...
    def visit_primary_key_constraint(
        ...
        using = constraint.get_dialect_option(self.dialect, "using", deprecated_fallback="mysql")
        if using:
...
class MySQLDialect(default.DefaultDialect):
    ...
    def initialize(self, connection: Connection) -> None:
        ...
        self.use_mysql_for_share = self._is_mysql and self.server_version_info >= (8, 0, 1)
    ...
    def _correct_for_mysql_bugs_88718_96365(
        ...
        if schema_by_table_by_column:
            ...
            correct_for_wrong_fk_case: CursorResult[Tuple[str, str, str]] = connection.execute(
                select
            )

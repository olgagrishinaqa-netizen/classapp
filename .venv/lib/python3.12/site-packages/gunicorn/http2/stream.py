...
class HTTP2Stream:
    ...
    def send_data(self, data, end_stream=False):
        ...
        """
        if not self.can_send:
            raise HTTP2StreamError(self.stream_id, f"Cannot send data in state {self.state.name}")
    ...
    def get_regular_headers(self):
        ...
        """
        return [(name, value) for name, value in self.request_headers if not name.startswith(":")]

...
def peer_trusted_for_h2c(cfg, peer_addr):
    ...
    if not isinstance(peer_addr, tuple):
        return True
    return _ip_in_allow_list(peer_addr[0], cfg.forwarded_allow_ips, cfg.forwarded_allow_networks())


def _h2c_available(cfg):
    """Whether cleartext HTTP/2 could apply to this server at all."""
    return (
        "h2" in cfg.http_protocols and getattr(cfg, "protocol", "http") == "http" and not cfg.is_ssl
    )

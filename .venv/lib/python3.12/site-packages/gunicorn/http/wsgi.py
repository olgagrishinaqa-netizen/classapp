...
def create(req, sock, client, server, cfg, response_class=None, response_args=()):
...
class Response:
    ...
    def start_response(self, status, headers, exc_info=None):
        ...
        self._omits_body = self._response_omits_body(self.req.method, self.status_code)
        if self._omits_body and self._response_forbids_content_length(self.status_code):
            self.headers = [(k, v) for k, v in self.headers if k.lower() != "content-length"]
            self.response_length = None
    ...
    def _response_omits_body(method, status):
        ...
        return (
            method == "HEAD" or status in (204, 304) or (status is not None and 100 <= status < 200)
        )

...
class LifespanManager:
    ...
    async def startup(self):
        ...
        try:
            await asyncio.wait_for(
                self._startup_complete.wait(), timeout=30.0  # Reasonable startup timeout
            )
    ...
    async def shutdown(self):
        ...
        try:
            await asyncio.wait_for(
                self._shutdown_complete.wait(), timeout=30.0  # Reasonable shutdown timeout
            )
        except asyncio.TimeoutError:
    ...
    async def _send(self, message):
        ...
        if msg_type == "lifespan.startup.complete":
            ...
            self._startup_complete.set()
            self.logger.debug("Received lifespan.startup.failed: %s", self._startup_error)
            ...
            self._shutdown_complete.set()
            self.logger.debug("Received lifespan.shutdown.failed: %s", self._shutdown_error)

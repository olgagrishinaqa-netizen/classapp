...
class ASGIProtocol(asyncio.Protocol):
    ...
    def _start_http1(self, buffered=b""):
        ...
        self._h2c_upgrade_ok = negotiation.upgrade_allowed(self.cfg, peername) and hasattr(
            self._callback_parser, "remaining"
        )
    ...
    def _on_headers_complete(self):
        ...
        if self._h2c_upgrade_ok:
            self._h2c_upgrade_settings = negotiation.upgrade_settings(self._current_request)
    ...
    async def _handle_h2c_upgrade(self, request):
        ...
        await self._handle_http2_connection(self.transport, None, upgrade=(settings, request, body))
    ...
    async def _handle_http2_connection(self, transport, ssl_object, upgrade=None):
        ...
        try:
            ...
            if upgrade is None:
                ...
                upgraded = await h2_conn.initiate_upgrade(settings, http1_req, body)
                self._h2_conn = h2_conn
                await self._serve_http2_request(upgraded, h2_conn, sockname, peername)
                self.worker.nr += 1
            ...
            while not h2_conn.is_closed and self.worker.alive:
                ...
                for req in requests:
                    await self._serve_http2_request(req, h2_conn, sockname, peername)

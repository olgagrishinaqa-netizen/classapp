...
class DirtyWorker:
    ...
    SIGNALS = [getattr(signal, "SIG%s" % x) for x in "ABRT HUP QUIT INT TERM USR1".split()]
    ...
    async def handle_request(self, message, writer):
        ...
        try:
            ...
            response = make_error_response(
                request_id, DirtyAppError(str(e), app_path=app_path, action=action, traceback=tb)
            )
            await DirtyProtocol.write_message_async(writer, response)
    ...
    async def _stream_sync_generator(self, request_id, gen, writer):
        ...
        try:
            ...
            while True:
                ...
                self.notify()
            # Send end message
            await DirtyProtocol.write_message_async(writer, make_end_message(request_id))
        except Exception as e:
            # Error during streaming - send error message
            tb = traceback.format_exc()
            self.log.error("Error during streaming: %s\n%s", e, tb)
            response = make_error_response(request_id, DirtyAppError(str(e), traceback=tb))
            await DirtyProtocol.write_message_async(writer, response)
        finally:
            gen.close()
    ...
    async def execute(self, app_path, action, args, kwargs):
        ...
        try:
            result = await asyncio.wait_for(
                loop.run_in_executor(self._executor, lambda: app(action, *args, **kwargs)),
                timeout=timeout,
            )
            return result
        except asyncio.TimeoutError:
            # Note: The thread continues running - we just stop waiting
            self.log.warning("Execution timeout for %s.%s after %ds", app_path, action, timeout)
            raise DirtyTimeoutError(f"Execution of {app_path}.{action} timed out", timeout=timeout)

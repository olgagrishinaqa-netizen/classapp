    ...
    def encode(value) -> bytes:  # pylint: disable=too-many-return-statements
        ...
        if isinstance(value, str):
            ...
            if len(encoded) > MAX_STRING_SIZE:
                raise DirtyProtocolError(
                    f"String too large: {len(encoded)} bytes " f"(max: {MAX_STRING_SIZE})"
                )
        ...
        raise DirtyProtocolError(f"Unsupported type for TLV encoding: {type(value).__name__}")
    ...
    def decode(data: bytes, offset: int = 0) -> tuple:  # pylint: disable=too-many-return-statements
        ...
        if type_code == TYPE_INT64:
            if offset + 8 > len(data):
                raise DirtyProtocolError(
                    "Truncated TLV data: incomplete int64", raw_data=data[offset - 1 : offset + 20]
                )
        ...
        if type_code == TYPE_BYTES:
            ...
            if length > MAX_BYTES_SIZE:
                raise DirtyProtocolError(f"Bytes too large: {length} bytes (max: {MAX_BYTES_SIZE})")

            if offset + length > len(data):
                raise DirtyProtocolError(
                    f"Truncated TLV data: expected {length} bytes, " f"got {len(data) - offset}",
        ...
        if type_code == TYPE_LIST:
            ...
            if count > MAX_LIST_SIZE:
                raise DirtyProtocolError(f"List too large: {count} items (max: {MAX_LIST_SIZE})")
        ...
        if type_code == TYPE_DICT:
            ...
            if count > MAX_DICT_SIZE:
                raise DirtyProtocolError(f"Dict too large: {count} items (max: {MAX_DICT_SIZE})")

            result = {}
            for _ in range(count):
                key, offset = TLVEncoder.decode(data, offset)
                if not isinstance(key, str):
                    raise DirtyProtocolError(f"Dict key must be string, got {type(key).__name__}")
                value, offset = TLVEncoder.decode(data, offset)
                result[key] = value
            return result, offset

        raise DirtyProtocolError(
            f"Unknown TLV type code: 0x{type_code:02x}", raw_data=data[offset - 1 : offset + 20]
        )

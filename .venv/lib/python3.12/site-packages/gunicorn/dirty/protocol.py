...
class BinaryProtocol:
    ...
    def encode_header(msg_type: int, request_id: int, payload_length: int) -> bytes:
        ...
        """
        return struct.pack(HEADER_FORMAT, MAGIC, VERSION, msg_type, payload_length, request_id)
    ...
    def decode_header(data: bytes) -> tuple:
        ...
        if len(data) < HEADER_SIZE:
            raise DirtyProtocolError(
                f"Header too short: {len(data)} bytes, expected {HEADER_SIZE}", raw_data=data
            )
        ...
        if version != VERSION:
            raise DirtyProtocolError(
                f"Unsupported protocol version: {version}, expected {VERSION}", raw_data=data[:20]
            )

        if msg_type not in MSG_TYPE_TO_STR:
            raise DirtyProtocolError(f"Unknown message type: 0x{msg_type:02x}", raw_data=data[:20])

        if length > MAX_MESSAGE_SIZE:
            raise DirtyProtocolError(f"Message too large: {length} bytes (max: {MAX_MESSAGE_SIZE})")

        return msg_type, request_id, length

    @staticmethod
    def encode_request(
        request_id: int, app_path: str, action: str, args: tuple = None, kwargs: dict = None
    ) -> bytes:
        """
        ...
        header = BinaryProtocol.encode_header(MSG_TYPE_REQUEST, request_id, len(payload))
    ...
    def encode_response(request_id: int, result) -> bytes:
        ...
        payload = TLVEncoder.encode(payload_dict)
        header = BinaryProtocol.encode_header(MSG_TYPE_RESPONSE, request_id, len(payload))
        return header + payload
    ...
    def decode_message(data: bytes) -> tuple:
        ...
        if len(data) < HEADER_SIZE + length:
            raise DirtyProtocolError(
                f"Incomplete message: expected {HEADER_SIZE + length} bytes, " f"got {len(data)}",
                raw_data=data[:50],
            )
    ...
    async def read_message_async(reader: asyncio.StreamReader) -> dict:
        ...
        try:
            ...
            if len(e.partial) == 0:
                ...
                raise
            raise DirtyProtocolError(
                f"Incomplete header: got {len(e.partial)} bytes, " f"expected {HEADER_SIZE}",
                raw_data=e.partial,
            )
        ...
        if length > 0:
            try:
                ...
                raise DirtyProtocolError(
                    f"Incomplete payload: got {len(e.partial)} bytes, " f"expected {length}",
                    raw_data=e.partial,
                )
    ...
    def _recv_exactly(sock: socket.socket, n: int) -> bytes:
        ...
        while len(data) < n:
            ...
            if not chunk:
                ...
                raise DirtyProtocolError(
                    f"Connection closed after {len(data)} bytes, expected {n}", raw_data=data
                )
            data += chunk
        return data
...
def make_stash_message(request_id, op: int, table: str, key=None, value=None, pattern=None) -> dict:
    """

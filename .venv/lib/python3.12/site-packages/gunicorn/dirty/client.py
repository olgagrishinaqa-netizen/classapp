...
class DirtyClient:
    ...
    def _execute_locked(self, app_path, action, args, kwargs):
        ...
        request = make_request(
            request_id=request_id, app_path=app_path, action=action, args=args, kwargs=kwargs
        )
        ...
        try:
            ...
            self._close_socket()
            raise DirtyTimeoutError("Timeout waiting for dirty app response", timeout=self.timeout)
...
class DirtyAsyncStreamIterator:
    ...
    async def _read_next_chunk(self):
        ...
        if now >= self._deadline:
            self._exhausted = True
            raise DirtyTimeoutError("Stream exceeded total timeout", timeout=self.client.timeout)
        ...
        try:
            ...
            if remaining > self._TIMEOUT_THRESHOLD:
                ...
                read_timeout = min(remaining, self._idle_timeout)
                response = await asyncio.wait_for(
                    DirtyProtocol.read_message_async(self.client._reader), timeout=read_timeout
                )
        except asyncio.TimeoutError:
            self._exhausted = True
            now = time.monotonic()
...
_async_client_var: contextvars.ContextVar[DirtyClient] = contextvars.ContextVar("dirty_client")

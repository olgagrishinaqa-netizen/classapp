...
print(
    f"py={sys.version.split()[0]} gil={getattr(sys, '_is_gil_enabled', lambda: True)()} "
    f"greenlet={greenlet.__version__} status={status}",
    flush=True,
)
if status == "collected":
    raise SystemExit(
        "REGRESSED: class reachable only through a suspended "
        "greenlet's C-stack ref was collected early"
    )
if status == "masked":
    raise SystemExit(
        "REGRESSED: class stayed alive but not through the suspended "

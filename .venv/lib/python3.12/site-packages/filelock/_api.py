...
def _same_exception_tree(first: BaseException, second: BaseException) -> bool:
    ...
    while pending:
        ...
        if first_group.message != second_group.message or len(first_group.exceptions) != len(
            second_group.exceptions
        ):
            return False
...
def _append_exception_context(error: BaseException, context: BaseException) -> None:
    ...
    while id(tail) not in seen:
        seen.add(id(tail))
        if (
            next_error := tail.__cause__ if tail.__cause__ is not None else tail.__context__
        ) is None:
...
class FileLockMeta(ABCMeta):
    ...
    def _create_instance(
        ...
        unsupported = sorted(
            ...
            and (
                (parameter := model.default_params.get(name)) is None or value != parameter.default
            )
        )
        if unsupported:
...
_INIT_PARAMETER_MODELS: Final[WeakKeyDictionary[type[BaseFileLock], _InitParameterModel]] = (
    WeakKeyDictionary()
)
...
class BaseFileLock(
    ...
    def _descriptors_for_fork(
        ...
        if self._context.pending_lock_file_fd is not None:
            descriptors.append(
                (self._context.pending_lock_file_fd, self._context.pending_lock_file_fd_identity)
            )
    ...
    def _rollback_failed_registration(
        ...
        if tracking_error is not None:  # pragma: no cover - requires failed in-memory fallback
            _raise_cleanup_errors(
                "descriptor registration cleanup failed", registration_error, tracking_error
            )
    ...
    def _register_unverified_context_descriptor(self) -> None:
        ...
        if (
            self._context.lock_file_fd is not None and self._context.lock_file_fd_token is None
        ):  # pragma: no branch

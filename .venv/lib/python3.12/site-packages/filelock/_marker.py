...
def _build_record(fields: dict[str, str]) -> OwnerRecord | None:
    ...
    if mode == "lease" and (
        token is None or duration is None or not (math.isfinite(duration) and duration > 0)
    ):
        return None
    return OwnerRecord(
        pid=pid, hostname=hostname, mode=mode, token=token, lease_duration=duration, start=start
    )

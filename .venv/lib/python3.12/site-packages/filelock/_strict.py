...
class StrictSoftFileLock(BaseFileLock):
    ...
    def _acquire(self) -> None:
        ...
        if (
            sentinel_fd := _open_or_create_sentinel(self.lock_file, lock_path, self._open_mode())
        ) is None:
            return
        try:
            ...
            try:
                ...
                _raise_cleanup_errors(
                    "strict sentinel inspection cleanup failed", inspection_error, close_error
                )
            raise
    ...
    while True:
        ...
        if deadline is None:
            ...
            if pending.errno == ESTALE:
                ...
                return None
            reason = (
                f"cannot read claim: {pending.strerror or str(pending) or type(pending).__name__}"
            )
...
def _parse_claim(
    ...
    return StrictSoftFileClaim(
        name=name, state=name_parts[0], token=token, pid=pid, hostname=hostname, start=start
    )
...
def _publish_record(
    ...
    try:
        ...
        try:
            ...
            _raise_cleanup_errors(
                "strict publication directory cleanup failed", publication_error, close_error
            )
        raise
    if directory_fd is not None:  # pragma: needs dir-fd
        try:
...
def _private_public_name(private_name: str) -> str | None:
    if not private_name.startswith(".") or not private_name.endswith(_PRIVATE_RECORD_SUFFIX):
        return None
    public_name, marker, random_hex = private_name[1 : -len(_PRIVATE_RECORD_SUFFIX)].rpartition(
        _PRIVATE_RECORD_MARKER
    )
    if (
...
def _link_relative(
    ...
    if directory_fd is not None and _LINK_SUPPORTS_DIR_FD:  # pragma: needs dir-fd
        _link_no_follow(
            source_name, destination_name, src_dir_fd=directory_fd, dst_dir_fd=directory_fd
        )
        return
    _link_no_follow(
